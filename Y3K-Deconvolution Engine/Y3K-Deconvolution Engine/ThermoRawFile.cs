...
namespace Y3K_Deconvolution_Engine
{
    public class ThermoRawFile : MSDataFile<ThermoSpectrum>
    {
        ...
        public Dictionary<string, double> GetScanParams(int scanNum)
        {
            ...
            int pnArraySize = -1;
            _rawConnection.GetStatusLogForScanNum(scanNum, ref pdStatusLogRT, ref pvarLabels, ref pvarValues, ref pnArraySize);
        ...
        public double GetLockMassAdjustment(int spectrumNumber)
        {
            object corr = null;
            string label = "LM m/z-Correction (ppm):";
            _rawConnection.GetTrailerExtraValueForScanNum(spectrumNumber, label, ref corr);
            return double.Parse(corr.ToString());
        }

        public ThermoSpectrum GetLockMassCorrectedSpectrum(int spectrumNumber)
        {
            ThermoSpectrum spectrum = GetLabeledSpectrum(spectrumNumber);
            double lockMassCorr = GetLockMassAdjustment(spectrumNumber);
            double[,] peakArray = new double[2, spectrum.Count];
            int count = 0;
            foreach (var peak in spectrum)
            {
                double adjustment = peak.MZ;
                adjustment = adjustment * ((lockMassCorr / 1e6));
                double newMZ = peak.MZ - adjustment;
                peakArray[0, count] = newMZ;
                peakArray[1, count] = peak.Intensity;
                count++;
            }
            var returnSpectrum = new ThermoSpectrum(peakArray, spectrum.Count);
            return returnSpectrum;
        }

        public DateTime GetCurrentInfo()
        {
            DateTime thisDataTime = DateTime.Now;
            _rawConnection.GetCreationDate(ref thisDataTime);
            return thisDataTime;
        }
    }
